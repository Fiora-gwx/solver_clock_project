...
def test_collect_shared_clock_velocity_norms_accepts_stork_pilot_sigma_domain() -> None:
    ...
    scheduler = build_scheduler("stork4_2nd")
    norms = collect_shared_clock_velocity_norms(
        model=model,
        scheduler=scheduler,
...
def test_collect_shared_clock_material_derivative_norms_accepts_stork_pilot_sigma_domain() -> None:
    class DummyModel(torch.nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.in_channels = 1
