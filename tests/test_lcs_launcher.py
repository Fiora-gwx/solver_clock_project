...
def _build_args() -> SimpleNamespace:
    return SimpleNamespace(
        ...
        clock_config="configs/clocks/LCS_1.yaml",
...
def test_canonical_schedule_name_accepts_lcs_aliases() -> None:
    ...
    try:
        canonical_schedule_name("LCS_2")
    except ValueError as error:
        assert "Unsupported schedule name" in str(error)
    else:
        raise AssertionError("LCS-2 should not be an accepted schedule alias.")
...
def test_materializable_schedule_registry_includes_lcs() -> None:
    ...
    assert not is_materializable_schedule("pndm", "LCS-2")
    assert not is_materializable_schedule("diffusers", "LCS-2")
...
def test_compact_metrics_filter_accepts_lcs_rows(tmp_path) -> None:
    ...
    assert not keep_row({"solver": "euler", "schedule": "linear"})


def test_profile_cache_dir_records_lcs_order_one() -> None:
    ...
    assert "order_1" in str(order_one)
    assert meta["schedule_family"] == "LCS-1"
    assert meta["lcs_order"] == 1
    assert meta["estimator"] == "material_derivative"
    assert meta["coordinate_domain"] == "timesteps"
