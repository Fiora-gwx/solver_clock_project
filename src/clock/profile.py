...
def _normalize_density_from_profile(alpha_profile: np.ndarray, physical_grid: np.ndarray) -> np.ndarray:
    ...
    if integral <= 0:
        raise ValueError("Clock alpha profile must have positive integral.")
...
def slice_profile_interval(
    ...
    *,
    ...
    if grid.ndim != 1 or alpha.ndim != 1 or len(grid) != len(alpha):
        raise ValueError("ClockProfile must contain aligned 1D physical_grid and alpha_profile arrays.")
    if len(grid) < 2:
        raise ValueError("ClockProfile must contain at least two grid points.")
