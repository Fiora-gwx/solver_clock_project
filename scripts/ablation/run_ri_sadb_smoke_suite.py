...
def _ri_case(
    *,
    ...
    return {
        ...
        "fixed_defect_solver": fixed_defect_solver,
    }


def winner_cases(args: argparse.Namespace) -> tuple[list[str], dict[str, str], list[dict[str, Any]]]:
    payload = load_json(args.winner_config)
    solvers = [str(name) for name in payload.keys()]
    mapping = {name: name for name in solvers}
    ...
    for solver in solvers:
        ...
        fixed = solver_payload.get("best_fixed_proxy") or {}
        if "eta" in fixed:
            cases.append(
                _ri_case(
                    solver=solver,
                    ...
                    defect_source=str(fixed.get("defect_source", "fixed_euler_proxy")),
                    fixed_defect_solver=args.fixed_defect_solver,
                )
            )
        target = solver_payload.get("best_target_defect") or {}
        if "eta" in target:
            cases.append(
                _ri_case(
                    solver=solver,
                    schedule="RI_SADB_TARGET_DEFECT",
                    eta=float(target["eta"]),
                    defect_mode="target_solver",
                    defect_source=str(target.get("defect_source", "target_stork_short_window" if is_stork_solver(solver) else f"target_{solver}")),
                    fixed_defect_solver=args.fixed_defect_solver,
                )
            )
    return solvers, mapping, cases
...
def write_clock_config(
    ...
    *,
    ...
    lines = [
        ...
        "  q_max: 6.0",
        "  warmup_steps: 1",
        "  cache_path: outputs/ri_sadb_smoke/cache/sadb_profiles",
...
def export_schedule(
    *,
    ...
    if case["schedule"] == "SADB":
        ...
        calibration_solver = args.fixed_defect_solver
    elif is_stork_solver(case["solver"]) and case.get("defect_source") == "stork_reference_fallback":
        calibration_solver = args.fixed_defect_solver
    ...
    bundle_dir = schedule_root / f"nfe_{case_nfe:03d}"
    patch_schedule_meta(
        bundle_dir,
        {
...
def aggregate_results(raw_rows: list[dict[str, Any]], diag_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in raw_rows:
        key = (
            str(row["dataset"]),
            str(row.get("model_asset", row.get("model", ""))),
            str(row["solver"]),
            str(row["schedule"]),
            str(row["ri_variant"]),
            str(row["eta"]),
    ...
    for key, rows in sorted(grouped.items()):
        ...
        diag_key = (key[2], key[3], key[4], key[5], key[6], key[7])
...
def best_eta(aggregate: list[dict[str, Any]], *, ri_variant: str, lower_is_better: bool = False) -> dict[str, str]:
    result: dict[str, str] = {}
    ...
    for solver, rows in by_solver.items():
        ...
        if not valid_rows:
            continue
        if lower_is_better:
            chosen = min(valid_rows, key=lambda row: float(row.get("metric_mean", float("inf"))))
        else:
...
def fixed_vs_target_table(aggregate: list[dict[str, Any]]) -> list[list[Any]]:
    ...
    for key in sorted(set(fixed) | set(target)):
        ...
        if "stork_reference_fallback" in {
            str(row.get("defect_source", ""))
            for row in aggregate
            if str(row.get("solver", "")) == key[0] and str(row.get("eta", "")) == key[1]
        }:
            winner = "not_conclusive"
        else:
...
def build_best_variants(aggregate: list[dict[str, Any]]) -> dict[str, Any]:
    payload: dict[str, Any] = {}
    solvers = sorted({str(row.get("solver", "")) for row in aggregate if row.get("solver")})
    for solver in solvers:
        payload[solver] = {"best_fixed_proxy": {}, "best_target_defect": {}}
        fixed_rows = [
            row
            for row in aggregate
            if row.get("solver") == solver
            and row.get("ri_variant") == "RI_SADB_FIXED_DEFECT"
            and float(row.get("success_rate", 0.0)) > 0.0
            and not math.isnan(float(row.get("fid_mean", float("nan"))))
        ]
        if fixed_rows:
            best = min(fixed_rows, key=lambda row: float(row["fid_mean"]))
            payload[solver]["best_fixed_proxy"] = {
                "eta": float(best["eta"]),
                "fid": float(best["fid_mean"]),
                "defect_source": best["defect_source"],
            }
        target_rows = [
            row
            for row in aggregate
            if row.get("solver") == solver
            and row.get("ri_variant") == "RI_SADB_TARGET_DEFECT"
            and float(row.get("success_rate", 0.0)) > 0.0
            and not math.isnan(float(row.get("fid_mean", float("nan"))))
        ]
        if target_rows:
            best = min(target_rows, key=lambda row: float(row["fid_mean"]))
            payload[solver]["best_target_defect"] = {
                "eta": float(best["eta"]),
                "fid": float(best["fid_mean"]),
                "defect_source": best["defect_source"],
            }
        if is_stork_solver(solver):
            sources = {str(row.get("defect_source", "")) for row in target_rows}
            if "target_stork_short_window" in sources:
                status = "native"
            elif "stork_reference_fallback" in sources:
                status = "fallback"
            else:
                status = "failed"
            payload[solver]["short_window_status"] = status
    return payload
...
def fid_value(rows: list[dict[str, Any]], solver: str, schedule: str, eta: str | None = None) -> float:
    candidates = [row for row in rows if row.get("solver") == solver and row.get("schedule") == schedule]
    if eta is not None:
        candidates = [row for row in candidates if str(row.get("eta", "")) == str(eta)]
    candidates = [row for row in candidates if not math.isnan(float(row.get("fid_mean", float("nan"))))]
    if not candidates:
        return float("nan")
    return float(candidates[0]["fid_mean"])


def generate_fid_report(
    *,
    path: Path,
    args: argparse.Namespace,
    device: str,
    ...
    eta_rows = [
        [
            ...
            row.get("defect_mode"),
            row.get("eta"),
            format_float(row.get("fid_mean")),
            format_float(row.get("fid_std")),
            row.get("num_samples"),
    ...
    stability_rows = []
    for solver in solvers:
        base = fid_value(aggregate, solver, "base")
        sadb = fid_value(aggregate, solver, "SADB")
        ri_g = fid_value(aggregate, solver, "RI_G", "1.0")
        stability_rows.append(
            [
                ...
                format_float(base),
                format_float(sadb),
                format_float(ri_g - base if not math.isnan(ri_g) and not math.isnan(base) else float("nan")),
                format_float(ri_g - sadb if not math.isnan(ri_g) and not math.isnan(sadb) else float("nan")),
            ]
        )
    fixed_target = fixed_vs_target_table(aggregate)
    fallback_used = any(row.get("status") == "FALLBACK" for row in raw_rows)
    native_stork = any(
        row.get("defect_source") == "target_stork_short_window" and row.get("status") == "OK"
        for row in raw_rows
    )
    blocker = (
        "STORKScheduler mutable history fields `noise_predictions`, `velocity_predictions`, `_step_index`, and `_begin_index` "
        "are not yet cloned/restored by a native augmented-state short-window collector."
        if any(is_stork_solver(solver) for solver in solvers) and not native_stork
        else ""
    )
    heun_adjusted = any(solver == "heun2" and nfe_for_solver(solver, int(args.nfe)) != int(args.nfe) for solver in solvers)
    recommendations = {}
    for solver, payload in best_variants.items():
        fixed = payload.get("best_fixed_proxy") or {}
        target = payload.get("best_target_defect") or {}
        candidates = [item for item in [fixed, target] if item.get("fid") is not None]
        recommendations[solver] = min(candidates, key=lambda item: float(item["fid"])) if candidates else {}
    lines = [
        "# RI-SADB CIFAR-10 FID Report",
        "",
        "## Environment",
        "",
        ...
        f"- git status: {dirty}",
        f"- metric: fid",
        f"- fid reference: {args.reference_stats_asset}",
        f"- num samples: {args.num_samples}",
        f"- batch size: {args.batch_size}",
        f"- heun2 NFE note: {'heun2 uses nfe=11 because vendor Heun requires odd effective NFE.' if heun_adjusted else 'not adjusted'}",
        "- commands executed:",
    ]
    ...
    lines.extend(
        [
            ...
            "## Eta Sweep",
            "",
            markdown_table(["solver", "defect_mode", "eta", "fid_mean", "fid_std", "num_samples", "success_rate"], eta_rows, limit=160),
            "",
            f"- best_eta_fixed_proxy: {json.dumps({k: v.get('best_fixed_proxy', {}) for k, v in best_variants.items()}, sort_keys=True)}",
            f"- best_eta_target_defect: {json.dumps({k: v.get('best_target_defect', {}) for k, v in best_variants.items()}, sort_keys=True)}",
            f"- best_eta_overall: {json.dumps(recommendations, sort_keys=True)}",
            "",
            "## RI_G Stability",
            "",
            markdown_table(["solver", "RI_G_fid", "base_fid", "SADB_fid", "RI_G_minus_base", "RI_G_minus_SADB"], stability_rows),
            "",
            "## Fixed Defect vs Target Defect",
            "",
            markdown_table(["solver", "eta", "fixed_proxy_fid", "target_defect_fid", "target_minus_fixed", "winner"], fixed_target, limit=160),
            "",
            "## STORK Short-Window",
            "",
            f"- native_short_window_stork_defect: {'yes' if native_stork else 'no'}",
            f"- fallback_used: {'yes' if fallback_used else 'no'}",
            f"- blocker: {blocker or 'none'}",
        ]
    )
    if any(is_stork_solver(solver) for solver in solvers) and not native_stork:
        lines.append("- STORK target-defect comparison is not conclusive because target short-window defect is still fallback.")
    lines.extend(
        [
            "",
            "## Final Recommendation",
            "",
            f"- recommended_eta_euler: {recommendations.get('euler', {}).get('eta', 'unavailable')}",
            f"- recommended_eta_heun2: {recommendations.get('heun2', {}).get('eta', 'unavailable')}",
            f"- recommended_eta_stork4_1st: {recommendations.get('stork4_1st', {}).get('eta', 'unavailable')}",
            f"- recommended_defect_mode: {json.dumps({k: v.get('defect_source', 'unavailable') for k, v in recommendations.items()}, sort_keys=True)}",
            "- next_experiment_sample_count: 10000 if runtime is acceptable; otherwise 5000.",
        ]
    )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def generate_report(
    *,
    ...
    lower_is_better = str(args.metric) == "fid" or bool(args.compute_fid)
    fixed_best = best_eta(aggregate, ri_variant="RI_SADB_FIXED_DEFECT", lower_is_better=lower_is_better)
    target_best = best_eta(aggregate, ri_variant="RI_SADB_TARGET_DEFECT", lower_is_better=lower_is_better)
    fixed_target = fixed_vs_target_table(aggregate)
...
def main() -> None:
    ...
    if args.winner_config:
        solvers, solver_mapping, cases = winner_cases(args)
    else:
            ...
            try:
                if args.schedule_only:
                    ...
                    if is_stork_solver(case["solver"]) and case["ri_variant"] == "RI_SADB_TARGET_DEFECT":
                        status = "FALLBACK"
                        error = "native STORK short-window RI-SADB target defect is not implemented; running fixed-proxy fallback schedule"
                        case = dict(case)
                        case["defect_source"] = "stork_reference_fallback"
                        row["defect_source"] = "stork_reference_fallback"
    ...
    best_variants = build_best_variants(aggregate)
    ...
    write_json(output_dir / "best_variants.json", best_variants)
    generate_report(
    ...
    generate_fid_report(
        path=output_dir / "ri_sadb_fid_report.md",
        args=args,
        device=device,
        env=env,
        git_commit=git_commit,
