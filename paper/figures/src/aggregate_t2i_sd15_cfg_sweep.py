...
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "outputs/gpde_diffusers_sd15_nfe10_cfg_seed_sweep"
DEFAULT_RESULTS_DIR = PAPER_ROOT / "results/t2i"
DEFAULT_TABLE = PAPER_ROOT / "tables/sd15_euler_nfe10_cfg_sweep_pairwise.tex"
DEFAULT_FIGURE = PAPER_ROOT / "figures/sd15_euler_nfe10_cfg_sweep_pairwise"

EXPECTED_GUIDANCE = (5.0, 7.5, 10.0)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Aggregate SD1.5 CFG-sweep text-to-image metrics.")
...
def validate_run_outputs(output_root: Path) -> dict[str, Path]:
    ...
    paths = {
        "summary": metrics_root / "gpde_diffusers_sd15_nfe10_cfg_seed_sweep.csv",
        "aggregate": metrics_root / "gpde_diffusers_sd15_nfe10_cfg_seed_sweep_aggregate.csv",
        "detail": metrics_root / "gpde_diffusers_sd15_nfe10_cfg_seed_sweep_detail.csv",
        "pairwise": metrics_root / "gpde_diffusers_sd15_nfe10_cfg_seed_sweep_pairwise.csv",
    ...
    schedule_paths = sorted(
        (
            output_root
            / "schedules/GPDE/diffusers/stable_diffusion_15/euler"
        ).glob("cfg_*/seed_*/nfe_010/schedule.json")
    )
    if len(schedule_paths) != len(EXPECTED_GUIDANCE) * len(EXPECTED_SEEDS):
        raise ValueError(f"Expected 9 D-GPDE schedules, found {len(schedule_paths)}.")
    ...
    if jpg_count != 1350 or manifest_count != 27:
...
def write_table(path: Path, rows: list[dict[str, str]]) -> None:
    ...
    lines = [
        r"\begin{table}[t]",
        r"  \centering",
        r"  \caption{SD1.5 text-to-image matched CFG sweep with Euler solver,",
        ...
        r"  \label{tab:sd15-euler-nfe10-cfg-sweep-pairwise}",
...
def copy_inputs(paths: dict[str, Path], output_root: Path, results_dir: Path) -> None:
    ...
    copies = {
        "detail": "sd15_euler_nfe10_cfg_sweep_detail.csv",
        "aggregate": "sd15_euler_nfe10_cfg_sweep_aggregate_by_seed.csv",
        "pairwise": "sd15_euler_nfe10_cfg_sweep_pairwise_raw.csv",
        "summary": "sd15_euler_nfe10_cfg_sweep_summary_raw.csv",
        "oracle_cost": "sd15_euler_nfe10_cfg_sweep_oracle_reuse_cost.csv",
    }
    ...
    for path in sorted(
        ...
        copy_sanitized_text(path, schedule_dir / f"sd15_euler_{cfg}_{seed}_nfe010_schedule.json")
...
def main() -> None:
    ...
    write_csv(
        args.results_dir / "sd15_euler_nfe10_cfg_sweep_schedule_summary.csv",
        list(schedule_rows[0].keys()),
        schedule_rows,
    )
    write_csv(
        args.results_dir / "sd15_euler_nfe10_cfg_sweep_pairwise_summary.csv",
