...
class MyCustomPipeline(DiffusionPipeline):
    ...
    def __call__(self, batch_size=4, num_inference_steps=50, height=32, width=32, generator=None):
        ...
        if init_noise_sigma is not None:
            noise = noise * torch.as_tensor(init_noise_sigma, device=device, dtype=noise.dtype)
        image = noise
        for t in self.scheduler.timesteps:
            t = t.unsqueeze(0)
            model_input = self.scheduler.scale_model_input(image, t) if hasattr(self.scheduler, "scale_model_input") else image
