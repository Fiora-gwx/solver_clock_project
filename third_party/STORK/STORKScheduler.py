...
class STORKScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(
        ...
        if self.prediction_type == "epsilon":
            self.set_timesteps_noise(num_inference_steps, device, sigmas=sigmas, timesteps=timesteps)
    ...
    def set_timesteps_noise(self,
        ...
        """
        self.num_inference_steps = num_inference_steps
        total_steps = float(self.config.num_train_timesteps)

        if timesteps is None:
            if sigmas is None:
                normalized_timesteps = np.linspace(0, 1, self.num_inference_steps + 1, dtype=np.float32)
                normalized_timesteps[0] = self.stopping_eps
                normalized_timesteps = normalized_timesteps[:-1]
                normalized_timesteps = normalized_timesteps[::-1]
                timestep_array = normalized_timesteps * total_steps
                timestep_array[-1] = self.stopping_eps * total_steps
                ...
                self.num_inference_steps = len(sigma_array)
                timestep_array = self._noise_timesteps_from_sigmas(sigma_array)
                normalized_timesteps = timestep_array / total_steps
        else:
            timestep_array = np.asarray(timesteps, dtype=np.float32).reshape(-1)
            if timestep_array.ndim != 1 or len(timestep_array) == 0:
                raise ValueError("Noise-based STORK custom timesteps must be a non-empty 1D array.")
            if np.any(np.diff(timestep_array) >= 0):
                raise ValueError("Noise-based STORK custom timesteps must be strictly descending.")
            self.num_inference_steps = len(timestep_array)
            normalized_timesteps = timestep_array / total_steps
            if sigmas is None:
                sigma_array = self._noise_sigmas_from_timesteps(timestep_array)
            else:
        ...
        self.sigmas = torch.from_numpy(
            np.concatenate([sigma_array.astype(np.float32), np.asarray([0.0], dtype=np.float32)])
    ...
    def _noise_sigmas_from_timesteps(self, timesteps: np.ndarray) -> np.ndarray:
        indices = np.arange(len(self.alphas_cumprod), dtype=np.float32)
        base_sigmas = torch.sqrt(
            torch.clamp(1.0 - self.alphas_cumprod, min=0.0) / torch.clamp(self.alphas_cumprod, min=1.0e-12)
        ).detach().cpu().numpy().astype(np.float32)
        clipped_timesteps = np.clip(np.asarray(timesteps, dtype=np.float32), indices[0], indices[-1])
        return np.interp(clipped_timesteps, indices, base_sigmas).astype(np.float32)


    def _noise_timesteps_from_sigmas(self, sigmas: np.ndarray) -> np.ndarray:
        indices = np.arange(len(self.alphas_cumprod), dtype=np.float32)
        base_sigmas = torch.sqrt(
            torch.clamp(1.0 - self.alphas_cumprod, min=0.0) / torch.clamp(self.alphas_cumprod, min=1.0e-12)
        ).detach().cpu().numpy().astype(np.float32)
        clipped_sigmas = np.clip(np.asarray(sigmas, dtype=np.float32), base_sigmas[0], base_sigmas[-1])
        return np.interp(clipped_sigmas, base_sigmas, indices).astype(np.float32)


    def _noise_dt_value(self, index: int, *, previous: bool = False) -> float:
    ...
    def step_noise_2(
        ...
        current_dt = self._noise_dt_value(self._step_index)
        dt = torch.ones(model_output.shape, device=sample.device) * current_dt
        ...
        if self._step_index == 0:
            ...
            # SECOND RUN
            t_previous = torch.ones(model_output.shape, device=sample.device) * self.timesteps[0] / total_step
            ...
            h = torch.as_tensor(
                0.5 * (self._noise_dt_value(1, previous=True) + self._noise_dt_value(2, previous=True)),
            ...
            h = torch.as_tensor(
                0.5 * (self._noise_dt_value(2, previous=True) + self._noise_dt_value(3, previous=True)),
                device=sample.device,
                dtype=model_output.dtype,
            )
